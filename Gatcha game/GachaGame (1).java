 ...
 * 
 * @author Eric Heisler, Prof. Parker, Prof. Martin, and Jiaqi Zou
 * @version 2023/ Mar 23
 */
public class GachaGame {

	/**
	 * This method is to uses the getRandomItem method to fill an array with howMany
	 * new Item objects.
	 * 
	 * @param howMany
	 * @return Array type newItems
	 */
	...
	/**
	 * This method creates a new Item object of one of the four subclasses at
	 * random. Use a Random object to generate a random number from 0 to 3
	 * (inclusive) to choose the subclass. Use the provided makeItemName method to
	 * create a random name. Use the Random object again to generate the needed
	 * statics for the item.
	 * 
	 * @param None
	 * @return Item type (Tool)&(Armor)&(Magic)&(Upgrade)
	 */
	public static Item getRandomItem() {
		Random random = new Random();
		int type = random.nextInt(4);
		if (type == 0) {
			return (Item) new Tool(makeItemName(0, random), random.nextInt(100) + 1);
		}
		if (type == 1) {
			return (Item) new Armor(makeItemName(1, random), random.nextInt(100) + 1);
		} else if (type == 2) {
			return (Item) new Magic(makeItemName(2, random), random.nextInt(100) + 1, random.nextInt(10) + 1);

		} else {
			return (Item) new Upgrade();
		}
	}
